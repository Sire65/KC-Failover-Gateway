...
async function sha256Base64Url(text) { return b64url(await crypto.subtle.digest('SHA-256', encoder.encode(String(text ?? '')))); }
function parseJsonObject(value) {
  if (!value) return null;
  let parsed; try { parsed = JSON.parse(value); } catch { return null; }
...
function cleanupReplayCache(nowSeconds) {
  ...
  let remove = replayCache.size - MAX_NONCE_CACHE;
  for (const key of replayCache.keys()) { replayCache.delete(key); if (--remove <= 0) break; }
}
export function allowedOrigin(request, env) {
  const origin = request.headers.get('origin');
  if (!origin) return null;
  const allowed = String(env.KC_ALLOWED_ORIGINS || '').split(',').map(x => x.trim()).filter(Boolean);
...
export async function buildCanonicalRequest(method, url, timestamp, nonce, bodyText) {
  const parsed = new URL(url); const bodyHash = await sha256Base64Url(bodyText || '');
...
export async function verifyEcdsaCanonical(publicJwk, canonical, signature) {
  ...
  let supplied; try { supplied = fromB64url(signature); } catch { return false; }
  try {
...
function basicHeaders(request) {
  return {
    deviceId:String(request.headers.get('x-kc-device-id')||''), keyVersionRaw:String(request.headers.get('x-kc-key-version')||'1'),
    timestampRaw:String(request.headers.get('x-kc-timestamp')||''), nonce:String(request.headers.get('x-kc-nonce')||''), signature:String(request.headers.get('x-kc-signature')||'')
  };
}
function validateHeaders(h, nowSeconds) {
  ...
  const timestamp=Number(h.timestampRaw); if(Math.abs(nowSeconds-timestamp)>MAX_SKEW_SECONDS) return {ok:false,status:401,code:'STALE_REQUEST'};
  return {ok:true,timestamp,keyVersion:Number(h.keyVersionRaw)};
}
async function verifyDeviceEntry(entry,h,canonical,keyVersion) {
  ...
  if (!await verifyEcdsaCanonical(entry.publicJwk,canonical,h.signature)) return {ok:false,status:401,code:'BAD_SIGNATURE'};
  return {ok:true,authMode:'ECDSA-P256',registerId:entry.registerId||null};
}
async function verifyLegacyHmac(keys,h,canonical) {
  const secret=keys?.[h.deviceId]; if(typeof secret!=='string'||secret.length<43) return {ok:false,status:401,code:'UNKNOWN_DEVICE'};
  const key=await crypto.subtle.importKey('raw',fromB64url(secret),{name:'HMAC',hash:'SHA-256'},false,['verify']);
  let supplied; try{supplied=fromB64url(h.signature)}catch{return {ok:false,status:401,code:'INVALID_SIGNATURE'}};
  return await crypto.subtle.verify('HMAC',key,supplied,encoder.encode(canonical)) ? {ok:true,authMode:'HMAC-SHA256',registerId:null} : {ok:false,status:401,code:'BAD_SIGNATURE'};
}

export async function authenticateRequest(request, env, {
  nowSeconds=Math.floor(Date.now()/1000), enforceReplay=true, deviceResolver=null, replayStore=null
}={}) {
  const h=basicHeaders(request); const valid=validateHeaders(h,nowSeconds); if(!valid.ok)return valid;
  ...
  let verified;
  if (deviceResolver) {
    let entry; try { entry=await deviceResolver(h.deviceId); } catch { return {ok:false,status:503,code:'SECURITY_STORE_UNAVAILABLE'}; }
    verified=await verifyDeviceEntry(entry,h,canonical,valid.keyVersion);
  } else {
    const registry=parseDevicePublicKeys(env), legacy=parseDeviceKeys(env);
    if (!registry && !legacy) return {ok:false,status:503,code:'SECURITY_NOT_CONFIGURED'};
    verified=registry ? await verifyDeviceEntry(registry[h.deviceId],h,canonical,valid.keyVersion) : await verifyLegacyHmac(legacy,h,canonical);
  }
  if(!verified.ok)return verified;

  if(enforceReplay){
    if(replayStore){
      let reserved; try { reserved=await replayStore({deviceId:h.deviceId,nonce:h.nonce,timestamp:valid.timestamp,ttlSeconds:MAX_SKEW_SECONDS+5}); }
      catch { return {ok:false,status:503,code:'SECURITY_STORE_UNAVAILABLE'}; }
      if(!reserved)return {ok:false,status:409,code:'REPLAY_DETECTED'};
    } else {
      cleanupReplayCache(nowSeconds); const replayKey=`${h.deviceId}:${h.nonce}`;
      if(replayCache.has(replayKey))return {ok:false,status:409,code:'REPLAY_DETECTED'};
      replayCache.set(replayKey,nowSeconds+MAX_SKEW_SECONDS+5);
    }
  }
